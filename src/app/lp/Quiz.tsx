    ...
    <div className="max-w-4xl mx-auto">
      <ScrollAnimation>
        <div className="text-center">
          <div className="inline-flex items-center gap-2 rounded-full bg-indigo-100/70 px-3 py-1 text-xs font-medium text-indigo-700 ring-1 ring-indigo-200 dark:bg-indigo-400/10 dark:text-indigo-200 dark:ring-indigo-400/30">
            <span>Responda em menos de 30s</span>
          </div>
          <h3 className="mt-4 text-3xl sm:text-4xl font-extrabold text-secondary-900 dark:text-white">
      ...
      {/* Progress */}
      <div className="mt-8">
        <div className="h-2 w-full rounded-full bg-secondary-200 dark:bg-secondary-700 overflow-hidden">
          <motion.div
            className="h-full bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-500"
            initial={{ width: 0 }}
            animate={{ width: `${(Math.min(step, totalSteps) / totalSteps) * 100}%` }}
            transition={{ type: 'spring', stiffness: 60, damping: 20 }}
          />
        </div>
        <div className="mt-2 text-xs text-secondary-500 dark:text-secondary-400 text-right">
        ...
        <AnimatePresence mode="wait">
          {step === 0 && (
            <motion.div key="step-0" variants={fadeUp} initial="initial" animate="animate" exit="exit">
              <Card className="p-6">
                <h4 className="text-xl font-semibold">O que você precisa agora?</h4>
                <p className="mt-1 text-sm text-secondary-600 dark:text-secondary-300">Escolha um serviço.</p>
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <button onClick={() => select('service', 'websites')} className="group rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500 focus-visible:ring-2">
                    <div className="text-left">
                      <div className="text-base font-medium">Desenvolvimento Web</div>
                      <div className="mt-1 text-xs text-secondary-600 dark:text-secondary-400">Sites e landing pages</div>
                    </div>
                  </button>
                  <button onClick={() => select('service', 'automation')} className="group rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500 focus-visible:ring-2">
                    <div className="text-left">
                      <div className="text-base font-medium">Automação & IA</div>
                      <div className="mt-1 text-xs text-secondary-600 dark:text-secondary-400">Fluxos e integrações</div>
                    </div>
                  </button>
                  <button onClick={() => select('service', 'consulting')} className="group rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500 focus-visible:ring-2">
                    <div className="text-left">
                      <div className="text-base font-medium">Consultoria Técnica</div>
          ...
          {step === 1 && answers.service === 'websites' && (
            <motion.div key="step-1-web" variants={fadeUp} initial="initial" animate="animate" exit="exit">
              <Card className="p-6">
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <button onClick={() => select('projectType', 'landing')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Landing page</button>
                  <button onClick={() => select('projectType', 'institutional')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Site institucional</button>
                  <button onClick={() => select('projectType', 'ecommerce')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">E-commerce / App sob medida</button>
                </div>
                <div className="mt-4">
          ...
          {step === 1 && answers.service === 'automation' && (
            <motion.div key="step-1-auto" variants={fadeUp} initial="initial" animate="animate" exit="exit">
              <Card className="p-6">
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <button onClick={() => select('projectType', 'simple')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Tarefa simples</button>
                  <button onClick={() => select('projectType', 'multi')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Fluxos multi-etapas</button>
                  <button onClick={() => select('projectType', 'ai_complex')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Integração complexa com IA</button>
                </div>
                <div className="mt-4">
                  <button onClick={prev} className="text-sm text-secondary-500 hover:text-secondary-700 dark:hover:text-secondary-200">Voltar</button>
                </div>
          ...
          {step === 1 && answers.service === 'consulting' && (
            <motion.div key="step-1-cons" variants={fadeUp} initial="initial" animate="animate" exit="exit">
              <Card className="p-6">
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <button onClick={() => select('projectType', 'diagnostic')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Diagnóstico (1h)</button>
                  <button onClick={() => select('projectType', 'package4h')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Pacote 4h</button>
                  <button onClick={() => select('projectType', 'package8h')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Pacote 8h</button>
                </div>
                <div className="mt-4">
                  <button onClick={prev} className="text-sm text-secondary-500 hover:text-secondary-700 dark:hover:text-secondary-200">Voltar</button>
                </div>
          ...
          {step === 2 && (
            <motion.div key="step-2" variants={fadeUp} initial="initial" animate="animate" exit="exit">
              <Card className="p-6">
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <button onClick={() => select('urgency', 'now')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Para esta semana</button>
                  <button onClick={() => select('urgency', 'soon')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Em 2–4 semanas</button>
                  <button onClick={() => select('urgency', 'exploring')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Estou explorando</button>
                </div>
                <div className="mt-4">
                  <button onClick={prev} className="text-sm text-secondary-500 hover:text-secondary-700 dark:hover:text-secondary-200">Voltar</button>
                </div>
          ...
          {step === 3 && (
            <motion.div key="step-3" variants={fadeUp} initial="initial" animate="animate" exit="exit">
              <Card className="p-6">
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <button onClick={() => select('budget', '1k')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Até R$ 1.000</button>
                  <button onClick={() => select('budget', '2k')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Até R$ 2.000</button>
                  <button onClick={() => select('budget', 'unknown')} className="rounded-lg border border-secondary-200 dark:border-secondary-700 p-4 hover:border-indigo-500">Não sei ainda</button>
                </div>
                <div className="mt-4">
                  <button onClick={prev} className="text-sm text-secondary-500 hover:text-secondary-700 dark:hover:text-secondary-200">Voltar</button>
                </div>
            ...
            <motion.div key="result" variants={fadeUp} initial="initial" animate="animate" exit="exit">
              <Card className="p-0 overflow-hidden">
                <div className="relative">
                  <div className="absolute inset-0 -z-10 bg-gradient-to-br from-indigo-600/10 via-fuchsia-600/10 to-cyan-600/10" />
                  <div className="p-6 sm:p-8">
                    ...
                    <h4 className="mt-3 text-2xl font-bold">{result.title}</h4>
                    ...
                    <div className="mt-8 flex flex-col sm:flex-row gap-3">
                      <a
                        ...
                        className="inline-flex items-center justify-center rounded-lg bg-primary-600 px-5 py-3 text-base font-semibold text-white shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:bg-primary-500 dark:hover:bg-primary-600"
                      >
                        {result.ctaLabel}
                      </a>
                      <a
